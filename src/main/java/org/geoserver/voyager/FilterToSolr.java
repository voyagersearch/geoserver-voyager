...
public class FilterToSolr implements FilterVisitor {
    ...
    private VoyagerConfig config;

    public FilterToSolr(SimpleFeatureType featureType, VoyagerConfig config) {
    ...
    public void encode(Filter filter) throws Exception {
        ...
        if (getCapabilities().fullySupports(filter)) {
            ...
            throw new Exception("Filter type not supported: " + filter);
        }
    }
    ...
    /** Sets the {@link FilterToSolr#uniqueId} */
    ...
    private Object visitBinarySpatialOperator(BinarySpatialOperator filter, Object extraData) {
        ...
        visitor.setSpatialStrategy(config.spatialStrategy);
